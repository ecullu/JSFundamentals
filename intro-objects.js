...
var getFullNames = function(objArray){
    ...
    for(var i = 0; i < objArray.length; i++){
        fullName.push(objArray[i].first + " " +objArray[i].last) 
    }
    return fullName
}
...
// with the letter 'r'.
var getFirstLetterR = function(str){
    
    return 'r' + str.substr(1,str.length);
};

dog.speak = function(wordStr){
    var splitted = wordStr.split(' ')
    var rWord = ''
    for (var i=0; i < splitted.length; i++){
        rWord += getFirstLetterR(splitted[i]) + ' '
       }
...
var getFirstLetterLower = function(str){
...
var getCounts = function(textStr){
    ...
    for (var i = 0; i < words.length; i++){
        var word = words[i]
        var wordLower = getFirstLetterLower(word)
        if(counterObj[word] === undefined && counterObj[wordLower] === undefined){
            counterObj[wordLower] = 1
           
        }
        else if (counterObj[word] === undefined && counterObj[wordLower] !== undefined ){
                counterObj[wordLower] = counterObj[wordLower] + 1
        }
        else {
            counterObj[wordLower] = counterObj[wordLower] + 1
...
// Write a function called reverseAllObjects(). It should take as input an array of 
...
var reverseAllObjects = function(arrOfObjects) {
    var allReversed = []
    for (var i = 0; i < arrOfObjects.length; i++){
        var reversed = reverseObject(arrOfObjects[i])
        allReversed.push(reversed)
    }
    return allReversed
...
var flippedUsers = reverseAllObjects(users)
...
var where = function(ObjList, ObjProp){
    var filtered = []
    for(var i = 0; i < ObjList.length; i++){
        for(var key in ObjList[i]){
            if(ObjList[i][key] === ObjProp[key]){
                filtered.push(ObjList[i])
            }
        }
    }
    return filtered
