...
// var nicer2 = function(input){
// 	var splitted = input.split(' ')
// 	var newStr = input.replace(/heck/gi, "")
// 	return newStr	
...
var capitalizeAll = function(input){
 	var strToArr = input.split(' ')
    var newStr = getFirstLetterCaps(strToArr[0])
 	for (var i = 1; i < strToArr.length; i++) {
 		var caped = getFirstLetterCaps(strToArr[i])
        newStr += ' ' + caped
 	}
 	return newStr
 }
...
var properSentences = function(input){
	var strToSent = input.split(". ")
	var newStr = getFirstLetterCaps(strToSent[0])
	for (var i = 1; i < strToSent.length; i++) {
 		var caped = getFirstLetterCaps(strToSent[i])
        newStr += '. ' +caped
 	}
 	return newStr
}
...
var iPutTheFunIn = function (input){
	...
	return input.slice(0,center) + 'fun' + input.slice(center,input.length)
}
